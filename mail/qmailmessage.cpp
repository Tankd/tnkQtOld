...
static QList<QByteArray> splitUtf8(const QByteArray& input, int maximumEncoded)
{
    ...
    qDebug() << Q_FUNC_INFO << "Need to cut the UTF-8 string !!!";
    ...
    do {
        ...
        while (!isFirstCodePoint(iter--)) {
              qDebug() << Q_FUNC_INFO << "Cutting point not valid, looking for the previous one !!!";
            Q_ASSERT (index >= 0);
            index--;
        }
        ...
        qDebug() << Q_FUNC_INFO << "The STR is still too long !!!";
...
bool QMailMessageMetaData::setContentScheme(const QString &scheme)
{
    if (!impl(this)->_contentScheme.isEmpty() && (impl(this)->_contentScheme != scheme)) {
       qDebug() << "Warning - modifying existing content scheme from:" << impl(this)->_contentScheme << "to:" << scheme;
    }
